...
| `ADMIN_KEY` | password gate for `/admin` (Questions + Reports) — default admin login |
...
│   ├── questions.js        # All questions — generated by /admin/questions imports; hand-editable too
│   └── assets/
│       └── style.css       # All styles
├── backend/
│   ├── app.py              # Flask app — public routes + /admin (auth, questions, reports)
│   ├── db.py                # DB abstraction (SQLite in dev, MySQL in prod) — submissions + admin_users
│   ├── email_sender.py      # Email (console in dev, Gmail API in prod)
│   ├── questions_excel.py   # Shared Excel <-> questions.js logic (CLI scripts + /admin/questions)
│   ├── reports.py           # Submission-data aggregation + Excel export for /admin/reports
│   ├── static/
│   │   └── admin.css        # Styles for /admin/* pages (separate from the public frontend/ CSS)
│   ├── templates/           # /admin/* pages: login, nav, questions, reports (+print view), users, forbidden
│   └── requirements.txt
├── database/
│   └── schema.sql          # MySQL schema for production
├── scripts/
│   ├── export_questions_to_excel.py  # CLI: dump all questions/options to OMI_Questions_Review.xlsx
│   └── import_questions_from_excel.py  # CLI: import a reviewed workbook back into questions.js
├── OMI_Questions_Review.xlsx  # Generated — for content review, re-run the export script/button after edits
├── .env.example            # All config variables with comments
├── start_all.py             # Installs deps + creates .env files + runs all 3 services with one command
└── README.md
```
...
Password- (`ADMIN_KEY`) or Google-OAuth-gated, same dual-mode pattern as
omi-deepdive (see its README for the OAuth setup steps — identical here, just
with OMI's own three roles instead of omi-deepdive's four): `admin` (full
access), `content_editor` (Questions only), `analyst` (Reports only). Roles
only matter once OAuth is on; the shared `ADMIN_KEY` grants full access
either way, same as every other admin surface in this repo.

- **`/admin/questions`** — export all questions to Excel, edit offline, import
  back in. All-or-nothing: a bad row blocks the whole import (never a
  half-updated `questions.js`) and every problem is listed by sheet and row.
  The same logic backs `scripts/export_questions_to_excel.py` /
  `scripts/import_questions_from_excel.py` for anyone who'd rather script it.
- **`/admin/reports`** — aggregated, published-report-style analytics over
  OMI's own `submissions` table (sector/country/revenue-band/date-range
  filters) — separate from and not synced with omi-benchmarks. Only ever
  shows aggregates (mean/median/counts); individual respondent identity
  (email/name/role) is never queried by this feature. No sample-size
  suppression — every figure is shown, always paired with its `n` so a small
  sample is never mistaken for a reliable one. Export to Excel, or open the
  print-optimized view and use the browser's Print/Save-as-PDF (same
  approach as OMI's existing respondent-facing PDF report — no server-side
  PDF library in this repo).

---

## Editing questions

The primary path is **`/admin/questions`** in the browser (see above) — export
to Excel, edit, import back in. `frontend/questions.js` is regenerated from
that import, but isn't pure data: it also holds real logic
(`sectorArchetypeMap`, `resolveArchetype()`, `getSections()`) which an import
never touches, only the 5 question-data literals are rewritten.

Hand-editing `questions.js` directly still works too — useful context either way:
...
- After hand-editing, re-run `python scripts/export_questions_to_excel.py` (or the "Export to Excel" button on `/admin/questions`) to refresh `OMI_Questions_Review.xlsx` for content review — it reads directly from `questions.js` so it can drift out of sync if regenerated only some of the time.
